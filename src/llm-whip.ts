...
async function loadConfig(configPath: string): Promise<Config | null> {
  try {
    ...
    console.warn(
      `Unsupported config file type: ${configPath}. Please use JSON (.json) or JavaScript (.js) files.`,
    );
    return null;
  } catch (error) {
...
async function main() {
  ...
  if (args[0] === "audit") {
    ...
    const configPath =
      auditArgs.find((arg) => arg.startsWith("--config="))?.split("=")[1] ||
      (await findConfigFile());
    const format =
      (auditArgs.find((arg) => arg.startsWith("--format="))?.split("=")[1] as
        | "table"
        | "json"
        | "csv") || "table";
  ...
  if (args[0] === "watch") {
    ...
    const configPath =
      watchArgs.find((arg) => arg.startsWith("--config="))?.split("=")[1] ||
      (await findConfigFile());
    const grepFlag = watchArgs.find((arg) => arg.startsWith("--grep="));
    const grepPatterns = grepFlag
    ...
    if (directories.length === 0) {
      console.error(
        "Usage: llm-whip watch [directories...] [--config=path] [--grep=pattern1,pattern2]",
      );
      console.error(
        'Example: llm-whip watch ./src ./lib --config=config.ts --grep="TODO,FIXME"',
      );
      process.exit(1);
    }
    ...
    if (configPath) {
      try {
        ...
        if (customConfig) {
          ...
          console.error(
            "Config file must be a TypeScript (.ts) or JavaScript (.js) file",
          );
          process.exit(1);
        }
      } catch (err: unknown) {
