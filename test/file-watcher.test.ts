...
describe("FileWatcher", () => {
  ...
  test("should respect ignore patterns", () => {
    ...
    // @ts-ignore
    expect(watcher.shouldProcessFile("node_modules/package/test.ts")).toBe(false);
    // @ts-ignore
...
`;
    ...
    // @ts-ignore - accessing private method for testing
    const matches = watcher.checkPatterns("// TODO: implement this", "/test/file.ts", 3);
  ...
  test("should handle multiple patterns in same line", () => {
    ...
    // @ts-ignore
    const matches = watcher.checkPatterns("// TODO: fix this FIXME issue", "/test/file.ts", 1);
  ...
  test("should handle file processing without file system changes", async () => {
    ...
    await fs.writeFile(testFile, "// TODO: implement feature\nconsole.log('test');");
